...
def KaggleTestSetEvaluate(model_save_path,
    ...
    test_annotations_df["filepath"] = [os.path.join(data_dir,patientID)+'.png' for patientID in test_annotations_df.patientID]
    ...
    for index, each_img in enumerate(np.unique(test_annotations_df.filepath)): 
        ...
        tmp_img = evaluate_with_kaggle_metric.preprocess_input(tmp_img, backbone_name)
        ...
        if int(tmp_df.Target.iloc[0]) == 0: 
            ...
            gt_bboxes = [np.asarray((row.x, row.y, row.w, row.h),dtype=np.float) for rownum, row in tmp_df.iterrows()]
        ...
        for each_thres in score_threshold:
            ...
            # print(gt_bboxes)
            # print(box_array)
            mapiou = evaluate_with_kaggle_metric.mAP_IoU(gt_bboxes, box_array, scores)
            # Positives only 
            if len(gt_bboxes) != 0:
    ...
    results_df = results_df[["mAP", "threshold"]].groupby("threshold").mean().reset_index()
    ...
    results_df_positives_only = results_df_positives_only[["mAP", "threshold"]].groupby("threshold").mean().reset_index()
...
eval = KaggleTestSetEvaluate(str(MODEL_PATH[0]),str(backbone_name),TEST_ANNOTATIONS,image_min_side,TEST_IMAGES_DIR)
