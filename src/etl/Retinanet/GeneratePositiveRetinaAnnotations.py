...
def generate_annotations(folds_df, 
    ...
    for each_fold in range(SETTINGS_JSON['RETINANET_FOLDS']): 
        ...
        tmp_train_df["Target"] = ["Pneumonia" if target == 1 else None for target in tmp_train_df.Target]
        ...
        tmp_train_df_neg = tmp_train_df_neg.sample(n=int(num_unique_pos*percent_normal/pos_frac))
        ...
        tmp_valid_df["Target"] = ["Pneumonia" if target == 1 else None for target in tmp_valid_df.Target] 
        tmp_train_df.to_csv(save_annotations_dir.format(each_fold),header=False, index=False)
        tmp_valid_df.to_csv(save_validation_dir.format(each_fold), header=False, index=False)
...
generate_annotations(folds_df, data_dir, save_annotations_dir,save_validation_dir,0,None)
