...
def train(folds, backbone,
  ...
  command = "python {} --backbone {} --batch-size {}".format(
      TRAIN_KAGGLE_PATH, backbone, batch_size)
  command += " --gpu {} --stratified_folds {} --fold {} --data_dir {}".format(
      gpu, stratified_folds, fold, data_dir)
  command += " --steps {} --epochs {} --snapshot-path {}".format(
      steps, epochs, snapshot_path)
  command += " --image_min_side {} --image_max_side {} csv {}".format(
      image_min_side, image_max_side, annotations_folder)
  command += " {} --val-annotations {}".format(
      classes_file_path, val_annotations)
  os.system(command)
