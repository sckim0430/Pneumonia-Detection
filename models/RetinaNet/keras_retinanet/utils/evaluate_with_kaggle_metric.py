...
def KaggleEvaluate(model_save_path, 
    ...
    val_annotations_df.columns = ["filepath", "x1", "y1", "x2", "y2", "Target"]
    val_annotations_df["patientId"] = [os.path.splitext(filepath.split("/")[-1])[0] for filepath in val_annotations_df.filepath]
    
    folds_df = pd.read_csv(stratified_folds)
    folds_df = folds_df[folds_df.fold == fold]
    folds_df = folds_df.merge(val_annotations_df, on="patientId")
    folds_df["class_y"]  = [1 if _ == "Pneumonia" else 0 for _ in folds_df.Target]
    ...
    scores_df.to_csv("{}-scores-auc{}.csv".format(results_save_path.replace(".csv", ""), 
                                                            round(auroc, 3)))
